...
docker run -e ENABLE_COMPATIBILITY_REPORTING --env-file .travis/common/travis-ci.env -v $(pwd):/yuzu -v "$HOME/.ccache":/root/.ccache -u `stat -c "%u:%g" $(pwd)` quriouspixel/yuzu:latest /bin/bash /yuzu/.travis/appimage/docker.sh
